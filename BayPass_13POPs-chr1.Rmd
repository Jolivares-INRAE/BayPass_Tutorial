...
#conversion du .vcf
GWAS.pooldata <- vcf2pooldata(vcf.file = paste(path_vcf, "13pops-GATK-chr1-corrected.vcf.gz", sep=""), poolsizes = psizes_Z, poolnames = pnames, min.cov.per.pool = 4, min.rc = 2, max.cov.per.pool = 1e+06, min.maf = 0.05, remove.indels = FALSE, nlines.per.readblock = 1e+06)
...
```{r Distribution-XtX}
source("C:/BayPass_pipeline/utils/baypass_utils.R")
xtx.sorted=read.table(paste(path_out,"13pops-chr1-Core.GATK.xtx.sorted", sep=""),h=T)
