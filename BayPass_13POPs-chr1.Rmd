...
psizes_W <- as.numeric(c('60', '90', '16', '56', '90', '28','90', '80', '100', '28', '34', '20', '38')) #ploydie W/chr29
#conversion du .vcf
...
```{r Import}
XtX.res=read.table(paste(path_out, "GWAS-13pops-chr1_C8.xtx.merged.sorted", sep=""),h=T)
BF.res.Cov1=read.table(paste(path_out, "GWAS-13pops-chr1.Betai-Cov1.merged.sorted", sep=""),h=T)
C2.res.C1=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C1.merged.sorted", sep=""),h=T)
C2.res.C2=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C2.merged.sorted", sep=""),h=T)
C2.res.C3=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C3.merged.sorted", sep=""),h=T)
C2.res.C4=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C4.merged.sorted", sep=""),h=T)
C2.res.C5=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C5.merged.sorted", sep=""),h=T)
C2.res.C6=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C6.merged.sorted", sep=""),h=T)
C2.res.C7=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C7.merged.sorted", sep=""),h=T)
C2.res.C8=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C8.merged.sorted", sep=""),h=T)
C2.res.C9=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C9.merged.sorted", sep=""),h=T)
C2.res.C10=read.table(paste(path_out, "GWAS-13pops-chr1.contrast-C10.merged.sorted", sep=""),h=T)
...
thresh.C2=quantile(C2.res.C1$M_C2, probs=0.95)
thresh.BF<-c(15)
```

```{r plot, fig.width=20, fig.height=20}
#Manhattan plot simple des valeurs de C2 
...
#Manhattan plot sous 2 conditions: XtX>seuil & BF>seuil
#Manplot.thresh = ggplot(data=subset(Joined.res, M_XtX> thresh.XtX & M_C2>thresh.C2), aes(x=pos, y=M_C2)) + geom_point(aes(color=chr), alpha=0.8, size=1.5)
...
#write.table(List.bed, row.names = FALSE, file=paste(path_res, "List_Outliers.bed", sep=""), quote = FALSE, sep = "\t")
```
...
XtX_C2_Join1 = merge(x=XtX.res,y=C2.res.C2, by=c("chr","pos"))
XtX_C2_Join2 = merge(x=XtX.res,y=C2.res.C4, by=c("chr","pos"))
XtX_C2_Join3 = merge(x=XtX.res,y=C2.res.C3, by=c("chr","pos"))
XtX_C2_Join4 = merge(x=XtX.res,y=C2.res.C7, by=c("chr","pos"))
...
#Datmp3 <-read.table("C:/BayPass_pipeline/Resultats/Souches/chr1/candidats_2021-Top5.txt")
...
#Diagramme de Venn
vd <- venn.diagram(x=list("47-2R" = List.SNP1, "30-1R" = List.SNP2, "53-6R" = List.SNP3, "44-3R" = List.SNP4), fill = brewer.pal(4, "Set3"), cat.cex = 1.5, fontface = "bold", filename = NULL)
grid.newpage()
grid.draw(vd)
```
...
```{r List-Venn}
myV <- plotVenn(list("France" = List.SNP1, "Italie" = List.SNP2, "RGV" = List.SNP3, "13pops" = List.SNP4))
