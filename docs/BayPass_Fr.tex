...
\CommentTok{\#!/bin/bash}
\ExtensionTok{module}\NormalTok{ load bioinfo/bcftools{-}1.14}
\VariableTok{Path}\OperatorTok{=}\StringTok{\textquotesingle{}/..\textquotesingle{}}
...
\ControlFlowTok{do}
    \VariableTok{files}\OperatorTok{+=}\StringTok{" }\VariableTok{$Path}\StringTok{/project\_chr}\VariableTok{$\{i\}}\StringTok{.vcf.gz"}  
\ControlFlowTok{done}

\CommentTok{\# bcftools concatène la liste des vcf contenu dans la variable $file}
\ExtensionTok{bcftools}\NormalTok{ concat }\VariableTok{$files} \AttributeTok{{-}O}\NormalTok{ z }\AttributeTok{{-}o} \VariableTok{$Path}\NormalTok{/project\_chr2{-}28.vcf.gz}
\end{Highlighting}
\end{Shaded}
...
\VariableTok{Path}\OperatorTok{=}\StringTok{\textquotesingle{}/BayPass/Input\textquotesingle{}}
...
\NormalTok{{-}gfile }\VariableTok{$Path}\NormalTok{/}\VariableTok{$INPUT\_F1} \DataTypeTok{\textbackslash{} }                 \CommentTok{\#fichiers genobaypass.sub*}
\ExtensionTok{{-}poolsizefile} \VariableTok{$Path}\NormalTok{/poolsize }\DataTypeTok{\textbackslash{} }           \CommentTok{\#active le mode poolseq}
...
Pour le modèle standard, il faut rajouter au script précédent le fichier contenant les covariables quantitatives et l'option scalecov

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{{-}efile} \VariableTok{$Path}\NormalTok{/ecotype }\DataTypeTok{\textbackslash{} }                   \CommentTok{\#calcul des Bayes Factors: modèle standard \& auxiliaire}
\ExtensionTok{{-}scalecov} \DataTypeTok{\textbackslash{} }                              \CommentTok{\#re{-}scale les covariables quantitatives}
\ExtensionTok{{-}contrastfile} \VariableTok{$Path}\NormalTok{/contrast }\DataTypeTok{\textbackslash{} }           \CommentTok{\#calcul du contraste C2: modèle core, standard \& auxiliaire}
\end{Highlighting}
\end{Shaded}

Pour activer le modèle auxiliaire il faut ajouter l'

\begin{Shaded}
\begin{Highlighting}[]
\ExtensionTok{{-}auxmodel} \DataTypeTok{\textbackslash{} }                              \CommentTok{\#active le modèle auxiliaire}
\ExtensionTok{{-}omegafile} \VariableTok{$Path}\NormalTok{/omega.mat }\DataTypeTok{\textbackslash{} }             \CommentTok{\#matrice Ω modèle standard (optionnel) \& auxiliaire (obligatoire)}
