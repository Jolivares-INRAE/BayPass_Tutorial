...
#conversion du .vcf
GWAS.pooldata <- vcf2pooldata(vcf.file = "C:/BayPass_pipeline/vcf/chr1-9.vcf.gz", poolsizes = psizes, poolnames = pnames, min.cov.per.pool = 4, min.rc = 2, max.cov.per.pool = 1e+06, min.maf = 0.01, remove.indels = FALSE, nlines.per.readblock = 1e+06)
```
...
```{r Input-BayPass}
pooldata2genobaypass(GWAS.pooldata, writing.dir = "C:/BayPass_pipeline/Input/", subsamplesize = -1, subsamplingmethod = "thinning")
```
...
path<-"C:/BayPass_pipeline/Output/CTI/"
prefix <- "Poolseq.sub"
...
```{r Distribution-XtX}
source("C:/BayPass_pipeline/utils/baypass_utils.R")
xtx.sorted=read.table("C:/BayPass_pipeline/Output/CTI/Poolseq.xtx.merged.sorted",h=T)
...
```{r POD data}
source("C:/BayPass_pipeline/utils/baypass_utils.R")
POD.data=geno2YN("C:/BayPass_pipeline/Input/genobaypass")
pi.betaK=read.table("C:/BayPass_pipeline/Output/CTI/Poolseq.sub3_summary_beta_params.out",h=T)$Mean
...
```{r Thresholds}
source("C:/BayPass_pipeline/utils/baypass_utils.R")
path.POD <- "C:/BayPass_pipeline/Output/CTI/"
...
List.tresh.C2<-c()
for (j in 1:3) {
POD.C2.j=read.table(paste(path.POD, "Poolseq.POD_C2_",j,".summary_contrast.out", sep=""),h=T)$M_C2
...
path.results <- "C:/BayPass_pipeline/Output/CTI/"
...
write.table(Manplot.thresh$data, row.names = FALSE, file="C:/BayPass_pipeline/Output/CTI/C2_VS_pos_CTI.txt", quote = FALSE, sep = "\t")
...
write.table(List.bed, row.names = FALSE, file="C:/BayPass_pipeline/Output/CTI/List_Outliers.bed", quote = FALSE, sep = "\t")
```
