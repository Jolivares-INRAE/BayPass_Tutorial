...
for i in {1..26}; do sed -n "$i~26p" genobaypass > genobaypass.sub$i; sed -n "$i~26p" snpdet > snpdet.sub$i; done
...
L'exemple ci-dessous correspond à une analyse de type Pool-Seq, parrallélisée sur 50 CPU.
...
OUTPUT=${FILES[$SLURM_ARRAY_TASK_ID]/genobaypass/Poolseq_chr1}

baypass -gfile ~/chemin/$INPUT -poolsizefile ~/chemin/poolsize -d0yij 20 -contrastfile ~/chemin/contraste.txt -efile ~/chemin/ecotype.txt -outprefix $OUTPUT
