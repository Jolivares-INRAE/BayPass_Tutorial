...
write.table(Manplot.thresh$data, row.names = FALSE, file="C:/Bioinfo/Analyses_R/baypass_2.2/Output/CTI/C2_VS_pos_CTI.txt", quote = FALSE, sep = "\t")
...
write.table(List.bed, row.names = FALSE, file="C:/Bioinfo/Analyses_R/baypass_2.2/Output/CTI/List_Outliers.bed", quote = FALSE, sep = "\t")
```
...
#Diagramme de Venn
vd <- venn.diagram(x=list("Pop01" = Data_1, "pop02" = Data_2, "pop03" = Data_3), fill = brewer.pal(3, "Set3"), cat.cex = 1.5, fontface = "bold", filename = NULL)
grid.newpage()
grid.draw(vd)
```
...
```{r}
myV <- plotVenn(list("pop01" = Data_1, "pop02" = Data_2, "pop03" = Data_3))
myV <- plyr::ldply(listVennRegions(myV), cbind)
write.table(myV, file="C:/Bioinfo/Analyses_R/baypass_2.2/Output/CTI/overlap.txt", quote = FALSE, sep = "\t", row.names = FALSE)
#récupère la liste des intersects
myV %>% distinct(myV[,1])
...
List.tmp<-separate(plyr::ldply(paste(tmp[,2]), cbind), col = "1",  sep = "_", into = c("chr", "pos"))
List.bed2 <- plyr::ldply(paste(List.tmp[,1], as.numeric(List.tmp[,2])-1, List.tmp[,2]), cbind)
List.bed2<-mutate(separate(List.bed2, col = "1",  sep = " ", into = c("chr", "start", "end")))
write.table(Liste.bed2, file="C:/Bioinfo/Analyses_R/baypass_2.2/Output/CTI/overlap.bed", quote = FALSE, sep = "\t", row.names = FALSE)
