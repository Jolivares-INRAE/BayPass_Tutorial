...
XtX.BF = ggplot(data=subset(Joined.res, M_XtX> thresh.XtX & BF.dB.>thresh.BF), aes(x=pos, y=M_C2)) + geom_point(aes(color=chr), alpha=0.8, size=1.5)
XtX.BF + scale_x_continuous() + scale_y_continuous() + theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank())+ facet_grid(~chr,scales = 'free_x', space = 'free_x', switch = 'x')

#Exporte les données filtrées sous conditions
write.table(XtX.BF$data, row.names = FALSE, file="C:/Bioinfo/Analyses_R/baypass_2.2/Output/CTI/C2_VS_pos_CTI.txt", quote = TRUE, sep = " ")
```
...
#Diagramme de Venn
vd <- venn.diagram(x=list("Pop01" = Data_1, "pop02" = Data_2, "pop03" = Data_3), fill = brewer.pal(3, "Set3"), cat.col = c("darkgreen",  "darkblue", "darkred"), cat.cex = 1.5, fontface = "bold", filename = NULL)
grid.newpage()
grid.draw(vd)
```
...
write.table(myV, file="C:/Bioinfo/Analyses_R/baypass_2.2/Output/CTI/overlap_CTI.txt", quote = FALSE, sep = "\t", row.names = FALSE)
myV
```
