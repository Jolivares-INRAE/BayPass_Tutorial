...
library(ape)
```
Nous recommandons pour le variant calling l’utilisation des Samtools et de Varscan avec les paramètres de base, sauf la p-value qui est montée à 0.8 pour être le moins stringent possible. 
...
samtools mpileup -C 50 -d 5000 -q 20 -l ~/chemin/chr1.bed -f ~/chemin/reference.fas  -b BamList.txt | java -Xmx2G -jar /usr/local/bioinfo/src/VarScan/VarScan-2.4.2/VarScan.v2.4.2.jar mpileup2cns --variants --min-coverage 3 --min-avg-qual 20 --min-var-freq 0.001 --p-value 0.8 --output-vcf 1 --vcf-sample-list NameList.txt > ~/chemin/Poolseq.vcf
...
Lister les tailles haploïdes de chaque pop (2x nbr individus pour les diploïdes) dans un objet "psizes"

créer un fichier texte/tabulation "contraste" dans lequel on identifie sur une ligne l'appartenance de chaque pop à un groupe référence (-1), mutant (1) ou aucun groupe (0).
ex:
1 -1 0 0 -1 1

Créer un fichier texte/tabulation "ecotype" dans lequel on identifie une variable écologique pour chaque pop (une variable par ligne, plusieurs lignes possibles).
...
```{r Info-pops}
pnames <- as.character(c('17-47-002-R', '17-47-003-S', '17-49-001-S', '17-53-006-R', '18-84-001-R')) psizes <- as.numeric(c('180', '124', '114', '160', '184')) 
...
```{r plot-sliding-Fst, fig.width=20, fig.height=20}
...
```{r heatmap-PW-fst, fig.width=20, fig.height=20}
