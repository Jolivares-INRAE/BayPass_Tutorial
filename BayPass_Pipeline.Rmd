...
path_input <- "C:/BayPass_pipeline/Input/"
path_out <- "C:/BayPass_pipeline/Output/"
path_POD <- "C:/BayPass_pipeline/POD/"
path_res <- "C:/BayPass_pipeline/Resultats/"
```
...
psizes <- as.numeric(c('150', '180', '124', '114', '160', '184','150', '180', '124', '114', '160', '184','150')) 
#conversion du .vcf
GWAS.pooldata <- vcf2pooldata(vcf.file = paste(path_vcf, "chr1-9.vcf.gz", sep=""), poolsizes = psizes, poolnames = pnames, min.cov.per.pool = 4, min.rc = 2, max.cov.per.pool = 1e+06, min.maf = 0.01, remove.indels = FALSE, nlines.per.readblock = 1e+06)
```
...
```{r Distribution-XtX}
source("C:/BayPass_pipeline/utils/baypass_utils.R")
xtx.sorted=read.table(paste(path_out,"Poolseq.xtx.merged.sorted", sep=""),h=T)
...
```{r Thresholds}
source("C:/BayPass_pipeline/utils/baypass_utils.R")
POD.omega=as.matrix(read.table(paste(path_POD, "Poolseq.POD_mat_omega.out", sep="")))
plot(POD.omega,omegaB) ; abline(a=0,b=1)
FMD.POD <- fmd.dist(POD.omega,omegaB)
cat("Distance FMD =", FMD.POD, "\n")
#Seuil XtX top 1%
POD.XtX=read.table(paste(path_POD, "Poolseq.POD_summary_pi_xtx.out", sep=""),h=T)$M_XtX
...
List.tresh.C2<-c()
for (j in 1:3) {
POD.C2.j=read.table(paste(path_POD, "Poolseq.POD_C2_",j,".summary_contrast.out", sep=""),h=T)$M_C2
...
```{r Fusion-&-plot, fig.width=20, fig.height=20}
XtX.res=read.table(paste(path_out, "Poolseq.xtx.merged.sorted", sep=""),h=T)
BF.res.Cov1=read.table(paste(path_out, "Poolseq.Betai-Cov1.merged.sorted", sep=""),h=T)
C2.res.C1=read.table(paste(path_out, "Poolseq.contrast-C1.merged.sorted", sep=""),h=T)
C2.res.C2=read.table(paste(path_out, "Poolseq.contrast-C2.merged.sorted", sep=""),h=T)
C2.res.C3=read.table(paste(path_out, "Poolseq.contrast-C3.merged.sorted", sep=""),h=T)
...
write.table(Manplot.thresh$data, row.names = FALSE, file=paste(path_res, "XtX-BF_VS_pos.txt", sep=""), quote = FALSE, sep = "\t")
...
#Diagramme de Venn
vd <- venn.diagram(x=list("Contraste01" = List.SNP1, "Contraste02" = List.SNP2, "Contraste03" = List.SNP3), fill = brewer.pal(3, "Set3"), cat.cex = 1.5, fontface = "bold", filename = NULL)
grid.newpage()
grid.draw(vd)
```
...
```{r List-Venn}
myV <- plotVenn(list("Contraste01" = List.SNP1, "Contraste02" = List.SNP2, "Contraste03" = List.SNP3))
