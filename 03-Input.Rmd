...
La fonction **_pooldata2genobaypass_** (Chunk \@ref(exr:chunk3)) convertit l'ogbjet **_pooldata_** en fichier d'entrée pour BayPass:
...
```
<p style="font-family: calibri; font-size:11pt; font-style:italic; color:darkgreen">
   La commande **_sed_** prélève une ligne toutes les 6 et les copies dans un fichier **_.sub_** et s'exécute 6 fois pour balayer tous le fichier initial et créer ainsi 6 fichiers de sous-jeux de données.
</p>
```{bash scr3, eval=FALSE}
for i in {1..6}; do sed -n "$i~6p" genobaypass > genobaypass.sub$i; sed -n "$i~6p" snpdet > snpdet.sub$i; done
```
